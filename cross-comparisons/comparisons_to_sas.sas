...
data ipw2;
	...
	where dead ^= .;
run;
proc logistic data=ipw descending; 
...
data iptw; 
	...
	if art=1 then do;
		...
		* adding bounds;
		IF d < 0.1 THEN uwb = 1/0.1;
			ELSE IF d > 0.9 THEN uwb = 1/0.9;
			ELSE uwb = 1/d;
		IF d < 0.2 THEN uwb2 = 1/0.2;
			ELSE IF d > 0.9 THEN uwb2 = 1/0.9;
			ELSE uwb2 = 1/d;
	end;
		...
		* adding bounds;
		IF d < 0.1 THEN uwb = 1/0.9;
			ELSE IF d > 0.9 THEN uwb = 1/0.1;
			ELSE uwb = 1/(1-d);
		IF d < 0.2 THEN uwb2 = 1/0.8;
			ELSE IF d > 0.9 THEN uwb2 = 1/0.1;
			ELSE uwb2 = 1/(1-d);
	end;
run;
